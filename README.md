...
- As modificações realizadas no projeto acabaram atendendo somente o BackEnd, foi a primeira vez que tive contato com um projeto com essa estrutura AspNet MVC minha experiencia sempre foi atuando com Web APIs então tive algumas dificuldades para me ambientar ao projeto e a estrutura, por conta disso acabei focando meu tempo na area onde mais tenho dominio, no caso o BackEnd, e por fim nenhuma alteração foi feita no Front e nos testes.
- As modificações no BackEnd foram:
	- Separar a estruturas em camadas:
		- Aplicação: Onde ficam as controllers os objetos de respostas e o FrontEnd
		- Business: Onde ficam as services com as regras de negocio, e as Dtos das entidades para o retorno mais organizado do backend
		- Data: Onde ficam as estruturas de conecção com o banco de dados, as migrations, as models e os repositorios criados
		- Utils: Onde ficou localizado o mapeamento das entidades para a utilização do automapper
	- Criação de metodos base para os repositorios, serviços e controllers para facilitar e padronizar a escalabilidade do sistema
	- Criação de serviços para a diminuir a duplicidade de codigo
	- Criação de metodos de resposta padrão para facilitar o entendimento do frontend sobre o retorno do backend
	- Implementação do automapper para facilitar o mapeamento de entidades para dtos
